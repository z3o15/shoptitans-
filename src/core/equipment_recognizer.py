...
try:
    ...
    from src.core.advanced_matcher_standalone import (
    ...
    # 导入增强特征匹配器
    from src.core.enhanced_feature_matcher import (
...
class EnhancedEquipmentRecognizer:
    ...
    def __init__(self, default_threshold=80, algorithm_type="enhanced_feature",
        ...
        if algorithm_type == "enhanced_feature" and ENHANCED_FEATURE_MATCHER_AVAILABLE:
            try:
                # 延迟导入以避免循环导入
                from src.core.enhanced_feature_matcher import FeatureType, EnhancedFeatureEquipmentRecognizer
                ...
                print("✓ 增强特征识别器初始化成功")
                ENHANCED_FEATURE_MATCHER_AVAILABLE = True  # 标记为可用
           
            except Exception as e:
                print(f"❌ 增强特征识别器初始化失败: {e}")
                ENHANCED_FEATURE_MATCHER_AVAILABLE = False  # 标记为不可用
            ...
            if algorithm_type == "enhanced_feature" and not ENHANCED_FEATURE_MATCHER_AVAILABLE:
                print("警告: 增强特征匹配器不可用，回退到传统dHash算法")
                # 使用传统dHash算法
                algorithm_type = "traditional"
            elif algorithm_type == "advanced" and not ADVANCED_MATCHER_AVAILABLE:
                print("警告: 高级匹配器不可用，回退到传统dHash算法")
                # 使用传统dHash算法
                algorithm_type = "traditional"
            elif algorithm_type == "feature" and not FEATURE_MATCHER_AVAILABLE:
                print("警告: 特征匹配器不可用，回退到传统dHash算法")
                # 使用传统dHash算法
                algorithm_type = "traditional"
            else:
                if algorithm_type not in ["enhanced_feature", "advanced", "feature", "traditional"]:
